...
> 開発者プロトコル: [Chat エンドポイント統合](CHAT_ENDPOINT_INTEGRATION.ja-JP.md)
...
リポジトリには 7 つの代表サンプルがあり、二段抽出（ルート発見 + ハンドラ実装抽出）で検証済みです。

| 代表サンプル | ルート構文ファミリー | 現行アダプタのカバレッジ対象（同ファミリー） | 理論上の横展開（アダプタ拡張が必要） |
|---|---|---|---|
...
- 「現行アダプタのカバレッジ対象」は、同じ AST ルーティング構文を共有し、実装済み抽出ロジックで対応できる対象を示します。
- リポジトリで直接検証済みなのは 7 つの代表サンプル本体です: `backend/test/test_route_extractor_representative_samples.py`。
- 「理論上の横展開」は構文が近いため抽出可能性が高いことを示し、正式対応には専用アダプタ追加が必要です。

### AST 4 パラダイムへの正規化

発見処理は現在、次の 4 つの AST ルーティング・パラダイムに正規化されています。7 サンプルは「フレームワーク代表」であり、新しいパラダイムではありません。

- `decorator_metadata`: 注解/デコレータ由来のルート（FastAPI、Spring、ASP.NET Controller）
- `call_registration`: 呼び出し登録型（Express/Fastify、Gin/Echo/Fiber/Chi、ASP.NET Minimal API）
- `route_table`: 集中ルート表（Django URLConf）
- `imperative_dispatch`: 命令型制御フロー分岐（Node native `if/switch`）

4 種類とも同一の `RouteSnippet` に統一され、同じコード分割と LLM コンテキスト注入経路を通ります。

### 探索レイヤー全体フロー（条件分岐付き）

```mermaid
flowchart TD
     A[discover_project project_id, base_url, openapi_path, source_path] --> B{OpenAPI 取り込み成功?}
     B -- はい --> C[ingest_openapi RouteMap source=openapi を生成]
     B -- いいえ --> D{source_path は提供済み?}
     D -- いいえ --> E[探索失敗 OpenAPI エラーを返却]
     D -- はい --> F[ingest_semantic_routes AST セマンティック探索 RouteMap source=ast を生成]

     C --> G[generate_project_context]
     F --> G
     G --> H[build_capability_graph]

     H --> I{source_path は提供済み?}
     I -- いいえ --> J[ソース精密抽出をスキップ 全ルートを規則フォールバック]
     I -- はい --> K[RouteExtractor.extract_batch route_pairs]

     K --> L{アダプタ検出成功?}
     L -- いいえ --> M[全ルート snippet=None]
     L -- はい --> N[Adapter.extract_all_routes]

     N --> O{Tree-sitter と Query は利用可能?}
     O -- いいえ --> P[fallback_extract_all_routes]
     O -- はい --> Q[ソース走査 AST Query captures から RouteSnippet 生成]

     P --> R[対象ルートごとに照合]
     Q --> R

     R --> S{route_id 完全一致?}
     S -- はい --> T[候補から code 最長を採用]
     S -- いいえ --> U{path_matches で曖昧一致?}
     U -- はい --> T
     U -- いいえ --> V[当該ルート snippet=None]

     T --> W[命中片を約 32K で分割]
     V --> W
     M --> W

     J --> X[CapabilityGraph 組み立て]
     W --> Y{解析可能チャンクあり?}
     Y -- いいえ --> Z[analysis_map は空]
     Y -- はい --> AA[各チャンクを並列 LLM 解析]
     AA --> AB[analysis_map を統合]
     Z --> X
     AB --> X

     X --> AC{ルートに AI 解析あり?}
     AC -- はい --> AD[AI の domain safety summary を採用]
     AC -- いいえ --> AE[HTTP Method 規則でフォールバック]

     AD --> AF[RouteMap Capability Project 状態を保存]
     AE --> AF
     AF --> AG[探索完了]
```
...
9. カスタム GUI 対応のプラガブル chat プロトコル
...
10. OpenClaw との MCP 連携（マルチチャネル実行ゲートウェイ）
- OpenClaw の最大の価値は、自然言語だけで完全自動・無人運転の処理を進められることです。人が毎回クリックしなくても、AI が自分の端末・アカウント・チャネル上で動き続けます。
...
1. OpenClaw を起動し、自然言語自動化の入口として動かします。
2. OpenClaw 側で LUI-for-All を MCP ツールとして登録するか、OpenClaw のセッションを MCP クライアントへ橋渡しします。
3. LUI-for-All 側で MCP のトークンとゲートウェイ URL を設定し、OpenClaw から専属プロジェクトへ入れるようにします。
4. まずは読み取り専用の機能で接続確認し、その後に承認付きの業務操作を順番に追加します。
...
- `backend/.env` は `LUI_*` の全体設定用です（例: `LUI_DB_PATH`, `LUI_MCP_API_TOKEN`）。
- Matchbox の主鍵 `LLM_KEY` は `backend/.env` からは読み込みません。
- 初回起動後、`workspace/agent_matchbox/.env` が生成され、`LLM_KEY` はそこから読み込まれます。
- Matchbox の保存先を変更したい場合は、システム環境変数 `AGENT_MATCHBOX_HOME` を設定してください。

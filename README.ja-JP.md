...
| 代表サンプル | ルート構文ファミリー | 実測カバレッジ（現行アダプタ） | 理論上の横展開（アダプタ拡張が必要） |
|---|---|---|---|
| `fastapi_sample` | Python デコレータルーティング（`@router.get`, `@app.post`） | FastAPI、Flask、Sanic、Starlette、Litestar、Falcon、aiohttp、Tornado、Bottle、Quart | Ruby Sinatra/Grape、PHP Slim |
| `node_sample` | Node 呼び出しチェーン型（`app.get()`, `router.post()`） | Express、Fastify、Koa Router、Hono、Elysia、Restify、hapi | PHP Laravel/Lumen/Slim、Ruby Hanami |
...
| `node_native_sample` | 非フレームワーク手書きルーティング表（method/path -> handler） | Node.js built-in http | Python wsgiref/werkzeug 手書きルーティング、Ruby Rack、PHP Swoole |

注記:

- 「実測カバレッジ」は `backend/test/test_route_extractor_representative_samples.py` の検証結果に対応します。
